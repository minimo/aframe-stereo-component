    ...
    init: function(){
      ...
      if (isValidGeometry && this.material_is_a_video) {
        ...
        if (this.data.mode === "half") {
          const geometry = new THREE.SphereGeometry(geo_def.radius || 100, geo_def.segmentsWidth || 64, geo_def.segmentsHeight || 64, Math.PI / 2, Math.PI, 0, Math.PI);
        } else {
          const geometry = new THREE.SphereGeometry(geo_def.radius || 100, geo_def.segmentsWidth || 64, geo_def.segmentsHeight || 64);
        }
  ...
  'stereocam_component':{
    ...
    tick: function(time){
      ...
      if (!this.layer_changed) {
        ...
        const childrenTypes = [];
        this.el.object3D.children.forEach((item, index) => childrenTypes[index] = item.type);
